...
def extractId(input) :
	...
	for s in soup.findAll('bibl') :
		...
		try :
			...
			if b :
				if len(b.string.split()) > 1 :
					title = b.string
					#print title
				elif b.findNext('title') :
					title = b.string+b.findNext('title').string
					#print title
			elif c :
				title = c.string
				#print title
			...
			attach = 0
			for t in s.findAll(text=True) :
				nstr = str(t).split()
				for n in nstr :
					if prePunc.has_key(n) or attach == 1:
						refString = refString+n
						attach = 0
					else :
						refString = refString+' '+n
					if postPunc.has_key(n) :
						attach = 1
			#print tmp_str
		...
		title2 = urllib.quote(title.encode('utf-8'))
