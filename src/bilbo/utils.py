...
def simpleLabeling(line, type='bibl'):
    ...
    if type == 'note' : optStr = '-T -t note -d '
    elif type == 'p' : optStr = '-T -t p -d '
...
def labeling(line, modelname, options):
    ...
    if dtype == "note" : typeCorpus = 2
    elif dtype == "p" : typeCorpus = 3
...
def defaultOptions():
    parser = optparse.OptionParser(usage ='%prog [options] <input data folder> <output data folder>'
    ...
    common_opts = optparse.OptionGroup(parser, 'Training and labeling options','These options are for both training and labeling')
    common_opts.add_option('-t', '--typeref', dest="t", default="bibl", action="store", type='choice', choices=['bibl', 'note', 'p'], help="Input reference type")
    ...
    label_opts = optparse.OptionGroup(parser, 'Labeling options','These options are for labeling only')
