# -*- coding: utf-8 -*-
"""
Created on Tue Aug 25 15:27:22 2015
							...
							for c_tag in b.contents :
								...
								if len(c_tag) > 0  and c_tag != "\n" and c_tag != " " :
									...
									if (c_tag != c_tag.string) :	#if : if there is tag
                                                                                #print "there is a tag " , c_tag.string
                                                                                # mettre 1, le tag n'est pas déternimé par le présence de <bibl>
										...
										c_tag_str = string.split(c_tag)
                                                                                #print "there is NO tag " , c_tag_str
									...
									if len(b.attrs) :
										for key in b.attrs.keys() :
											if isinstance(b.attrs[key], unicode) : features.append(b.attrs[key])
                                                                        ...
                                                                        #print "COUCOU",input.encode('utf8'), b.name
									newWord = Word(input, [b.name], features)
