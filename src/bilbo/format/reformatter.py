...
features = [['ALLNUMBERS', 'NUMBERS'],	#1
			...
			['POSSEDITOR'],	#8
			['POSSPAGE'],	#9
			['SURNAMLIST', 'FORENAMELIST', 'PLACELIST']]	#10
...
allnumbers, numbers
dash
allcap, firstcap, allsmall, nonimpcap
initial
weblink
italic
posseditor
posspage
forenamelist, surnamelist, placelist
'''
...
def formatCRFPP(All_bibls, tr) :
	...
	for tmp_bibl in All_bibls : 
		...
		for tmp in tmp_bibl : #tmp is temp line in bibl
			tmp_features = ['NONUMBERS', 'NODASH', 'NONIMPCAP', 'NULL', 'NOINITIAL', 'NOWEBLINK', 'NOITALIC', 'NOEDITOR', 'NOPAGE', 'NOLIST']
