...
class CRF(object):
	...
	def runAnalyseTrain(self, corpus, typeCorpus, directory, fileRes) :
		...
		dependencyDir = os.path.join(self.rootDir, 'dependencies')
		#-threshold 0.5
		command = dependencyDir+"/tree-tagger-linux-3.2/bin/tree-tagger -token -no-unknown -lemma "+self.rootDir+"/KB/config/treetagger/french.par "+fileReadyAnalyse+" "+self.dirResult+fileRes
		process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE)
		process.wait()
		return
	...
	def runAnalyseBeforeTestEval(self, fileRes, PosDetails = 0) :
		"""
		reform the test file with result of TreeTagger
		"""

		fanalyseTreeTagger= open(self.dirResult+fileRes, 'r', encoding='utf-8')
		ftestWapiti= open(self.dirResult+'testdatawithlabel_CRF_Wapiti.txt', 'r', encoding='utf-8')
		fResultFinal= open(self.dirResult+'testdata_CRF_TreeTagger_WapitiEval.txt', 'w', encoding='utf-8')
