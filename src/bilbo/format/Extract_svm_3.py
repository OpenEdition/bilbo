...
class Extract_svm_3(Extract):
	...
	def transform (self, listofword_file, files_dir, argv3, argv4):
		#<argument1> : path to file that contains the list of words (obtained from the bag of words)
 		#<argument2> : Path the repository that contains the files to convert in SVM Light format
 		#<argument3> : Path to the file that will contains the descriptors (this file must end with ".dat")
 		#<argument4> : +1 for CR class; -1 for NCR class and ? for unknown class

                #print "extractor.transform", listofword_file, files_dir, argv3, argv4
    		with open(listofword_file, 'r', encoding="utf-8") as f:
        		attributes = f.read().splitlines()
    
    		list_files = glob.glob (files_dir+'*.txt')
    		feature_file = open(argv3, "w", encoding="utf-8")
    
    		feature_lines = []
    		for file in list_files :
        		try:
            
            			line = argv4 + " " # +1 --> CR / -1 --> NCR
	...
	def load_original(self, filename_ori, indices) :
		...
		"""
		flagEndRef = 0
		fouttr = open(os.path.join(self.rootDir, "Result/original_train.txt"), "w", encoding="utf-8")
		fouttst = open(os.path.join(self.rootDir, "Result/original_test.txt"), "w", encoding="utf-8")
		
		i = 0
		j=0
		for line in open(filename_ori, 'r', encoding="utf-8") :
			if len(line.split()) != 0 :
				if indices[i] == 1 :
	...
	def save_ID(self, tokens, features) :
		...
		f = open(os.path.join(self.rootDir, fname), 'w', encoding="utf-8")
		for k in tokens :
			f.write(str(k))
			f.write('\n')
		f.close()
	...
	def load_ID(self, tokens, features) :
		...
		del tokens[:]
		fname = "model/corpus3/"+self.options.m+"/inputID.txt"
		for line in open(os.path.join(self.rootDir, fname), 'r', encoding="utf-8") :
			n = line.split('\n')
			print(n)
			tokens.append(n[0])
